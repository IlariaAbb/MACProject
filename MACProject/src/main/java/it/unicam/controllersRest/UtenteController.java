...
public class UtenteController {
    ...
    public ResponseEntity<Object> addPOIAiPreferiti(Authentication authentication,
    ...
    public ResponseEntity<Object> addItinerarioAiPreferiti(Authentication authentication,
    ...
    @DeleteMapping("/atourist/removePOIAiPreferiti")
    public ResponseEntity<Object> removePOIAiPreferiti(Authentication authentication,
                                                       @RequestParam("IdPOI") Long idPOI) {
        Long idUtente = this.utenteAutenticatoRepository.findByUsername(authentication.getName()).getId();
        if (this.gestionePreferiti.removePOIDaiPreferiti(idUtente, idPOI)) {
            return new ResponseEntity<>("POI rimosso dai preferiti", HttpStatus.OK);
        } else {
            return new ResponseEntity<>("Il POI non era nei preferiti", HttpStatus.BAD_REQUEST);
        }
    }

    @DeleteMapping("/atourist/removeItinerarioAiPreferiti")
    public ResponseEntity<Object> removeItinerarioAiPreferiti(Authentication authentication,
                                                             @RequestParam("idItinerario") Long idItinerario) {
        Long idUtente = this.utenteAutenticatoRepository.findByUsername(authentication.getName()).getId();
        if (this.gestionePreferiti.removeItinerarioDaiPreferiti(idUtente, idItinerario)) {
            return new ResponseEntity<>("Itinerario rimosso dai preferiti", HttpStatus.OK);
        } else {
            return new ResponseEntity<>("L'itinerario non era nei preferiti", HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<Object> richiestaCambioRuolo(Authentication authentication) {
        Long id = this.utenteAutenticatoRepository.findByUsername(authentication.getName()).getId();
        if(this.gestioneRuolo.viewRichiesteCambioRuolo()
                .stream()
    ...
    public ResponseEntity<Object> cambioRuolo(@RequestParam("id") Long id,
    ...
    public ResponseEntity<Object> registrazioneUtente(@Valid @RequestBody UtenteAutenticatoGI utente) {
        ...
        this.registrazioneController.registrazioneUtente(utente.getEmail(),
                                                        utente.getUsername(),
                                                        PasswordEncoder.encode(utente.password()),
                                                        utente.getRuolo());
        return new ResponseEntity<>("ok", HttpStatus.OK);
    }
